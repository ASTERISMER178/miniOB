...
Tuple *ProjectOperator::set_current_tuple(Tuple *tuple) {
  tuple_.set_tuple(tuple);
  return &tuple_;
}

void ProjectOperator::add_projection(const Table *table, const FieldMeta *field_meta, bool is_tables)
{
  // 对单表来说，展示的(alias) 字段总是字段名称，
  // 对多表查询来说，展示的alias 需要带表名字
  TupleCellSpec *spec = new TupleCellSpec(new FieldExpr(table, field_meta));
  spec->set_alias(field_meta->name());
  spec->set_is_tables(is_tables);
