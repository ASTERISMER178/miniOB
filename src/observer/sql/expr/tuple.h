...
public:
  ...
  void set_is_tables(bool is_table) {
    is_tables_ = is_table;
  }

  const bool is_tables() const {return is_tables_; }
...
public:
  ...
  int cell_num() const override { return cell_num_; }

  RC cell_at(int index, TupleCell &cell) const override {return RC::UNIMPLENMENT; }
  RC  cell_spec_at(int index, const TupleCellSpec *&spec) const override {return RC::UNIMPLENMENT; }
  ...
  void remove_tuple() {
    // cell_num_ -= tuples_.back()->cell_num();
    tuples_.pop_back();
  }
private:
  int cell_num_;
