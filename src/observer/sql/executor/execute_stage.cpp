...
void print_tuple_header(std::ostream &os, const std::vector<Aggregate*> &aggregates) {
  const int aggr_num = aggregates.size();
  for (int i = aggr_num - 1; i >=0; --i) {
    if (i != aggr_num - 1) {
      os << " | ";
    }
    os << aggregates[i]->to_string();
  }
  if(aggr_num > 0) {
    os << '\n';
  }
...
void print_tuple_header(std::ostream &os, const ProjectOperator &oper)
{
  ...
  for (int i = 0; i < cell_num; i++) {
    ...
    if (cell_spec->alias()) {
      if(cell_spec->is_tables()) {
        os << ((FieldExpr *)cell_spec->expression())->table_name() << '.';
      }
      os << cell_spec->alias();
    }
  }
...
void print_tuple_header(std::ostream &os, const std::vector<Field> &query_fields) {
  const int field_num = query_fields.size();
  for (int i = 0; i < field_num; ++i) {
    if (i != 0) {
      os << " | ";
    }

    auto &field = query_fields[i];
...
void tuple_to_string(std::ostream &os, const CompositeTuple &com_tuple) {
  TupleCell cell;
  RC rc = RC::SUCCESS;
  bool first_field = true;
  for (auto tuple : com_tuple.tuples()) {
    for (int i = 0; i < tuple->cell_num(); ++i) {
      rc = tuple->cell_at(i, cell);
      if (rc != RC::SUCCESS) {
        LOG_WARN("failed to fetch field of cell. index=%d, rc=%s", i, strrc(rc));
...
RC ExecuteStage::do_select(SQLStageEvent *sql_event)
{
  ...
  SessionEvent *session_event = sql_event->session_event();
  RC rc = RC::SUCCESS;
  if (select_stmt->aggregates().size() > 0) {
    rc = do_aggregation(sql_event);
    return rc;
  }
  if (select_stmt->tables().size() > 1) {
    auto &tables = select_stmt->tables();
    DescartesOperator des_oper(select_stmt->filter_stmt());
    for (int i = 0; i < tables.size(); ++i) {
      Operator *scan_oper = new TableScanOperator(tables[i]);
      PredicateOperator *pred_oper = new PredicateOperator(select_stmt->filter_stmt());
      pred_oper->add_child(scan_oper);
      des_oper.add_child(pred_oper);
    }
    ...
    ProjectOperator proj_oper;
    proj_oper.add_child(&des_oper);
    for (auto & field : select_stmt->query_fields()) {
      proj_oper.add_projection(field.table(), field.meta(), true);
    }
    ...
    std::stringstream ss;
    print_tuple_header(ss, proj_oper);
    for(auto &tuple : des_oper.com_tuple_sets()) {
      Tuple *proj_tuple = proj_oper.set_current_tuple(&tuple);
      tuple_to_string(ss, *proj_tuple);
      ss << std::endl;
    }
...
RC ExecuteStage::do_aggregation(SQLStageEvent *sql_event)
{
  SelectStmt *select_stmt = (SelectStmt *)(sql_event->stmt());
  SessionEvent *session_event = sql_event->session_event();
  RC rc = RC::SUCCESS;
  ...
  auto &table = select_stmt->tables()[0];
  Operator *scan_oper = new TableScanOperator(table);

  AggregationOperator aggr_oper(select_stmt);
  ...
  std::stringstream ss;
  auto &aggregates = select_stmt->aggregates();
  print_tuple_header(ss, aggregates);
  int aggr_size = select_stmt->aggregates().size();
  for(int i = aggr_size-1; i >= 0; --i) {
    if (i != aggr_size-1) {
      ss << " | ";
    }
    aggregates[i]->value_to_string(ss);
  }
  ss << std::endl;
...
RC ExecuteStage::do_create_index(SQLStageEvent *sql_event)
{
  ...
  Trx *trx = new Trx();
  RC rc = table->create_index(trx, create_index.index_name, create_index.attribute_name, create_index.is_unique);
...
RC ExecuteStage::do_insert(SQLStageEvent *sql_event)
{
  ...
  Trx *trx = new Trx();
  auto insert_sets = insert_stmt->insert_sets();
  for (int i = 0; i < insert_stmt->values_amount(); ++i) {
    auto &insert_set = insert_sets[i];
    RC rc = table->insert_record(trx, insert_set.value_num, insert_set.values);
    if (rc != RC::SUCCESS) {
      trx->rollback();
      session_event->set_response("FAILURE\n");
      return rc;
