...
bool check_date(int y, int m, int d) {
  ...
  return y > 0 && m > 0 && m <= 13 && d > 0 && d <= mm[m] + (leap && m==2) ? 1 : 0;
}
void value_init_date(Value *value, const char *v) {
  value->type = DATES;
  int y, m, d;
  sscanf(v, "'%d-%d-%d'", &y, &m, &d);
  if(!check_date(y,m,d)) return value_init_string(value, v);
...
void selects_append_aggregation(Selects *selects, AggrType aggr_type, const char *aggregation) {
  auto &aggr_num = selects->aggregation_num;
  selects->aggr_type[aggr_num] = aggr_type;
  selects->aggregations[aggr_num++] = strdup(aggregation);
}
...
void inserts_append(Inserts *inserts, Value values[], size_t value_num)
{
  auto &values_num = inserts->values_num;
  InsertSet &insert_set = inserts->insert_sets[values_num];
  assert(value_num <= sizeof(insert_set.values) / sizeof(insert_set.values[0]));
  for (size_t i = 0; i < value_num; ++i) {
    insert_set.values[i] = values[i];
  }
  insert_set.value_num = value_num;
  inserts->values_num++;
}
...
void inserts_destroy(Inserts *inserts)
{
  ...
  for (size_t i = 0; i < inserts->values_num; ++i) {
    InsertSet &insert_set = inserts->insert_sets[i];
    for(size_t j = 0; j < insert_set.value_num; ++j) {
      value_destroy(&insert_set.values[j]);
    }
    insert_set.value_num = 0;
  }
...
void create_index_init(
    CreateIndex *create_index, const char *index_name, const char *relation_name, const char *attr_name, bool is_unique)
{
