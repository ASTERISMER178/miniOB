...
RC Table::drop(const char *path, const char *name, const char *base_dir)
{
  ...
  int ret = remove(path);
  if (ret != 0) {
    LOG_ERROR("Failed to remove meta file=%s, errno=%d", path, ret);
...
class IndexInserter {
  public:
  IndexInserter() {}
  virtual ~IndexInserter() = default;

  virtual RC insert_index(const Record *record) = 0;
};

class NormalIndexInserter : public IndexInserter {
  public:
  NormalIndexInserter(Index *index) : index_(index) {}

  RC insert_index(const Record *record) override {
    return index_->insert_entry(record->data(), &record->rid());
  }
  private:
  Index *index_;
...
class UniqueIndexInserter : public IndexInserter {
  public:
  UniqueIndexInserter(Index *index) : index_(index) {}
  RC insert_index(const Record *record) override {
    RC rc = index_->insert_entry(record->data(), &record->rid());
    if (rc != RC::SUCCESS) {
      // rollback
      for (auto rec : records) {
        RC rc2 = index_->delete_entry(rec.first, &rec.second);
        if (rc2 != RC::SUCCESS) {
          return rc2;
        }
      }
    ...
    RID rid = record->rid();
    records[record->data()] = rid;
    return rc;
  }
  private:
  Index *index_;
  std::unordered_map<const char*, RID> records;
};
...
RC Table::create_index(Trx *trx, const char *index_name, const char *attribute_name, bool is_unique)
{
  ...
  // 遍历当前的所有数据，插入这个索引
  IndexInserter *index_inserter = nullptr;
  if (is_unique) index_inserter = new UniqueIndexInserter(index);
  else index_inserter = new NormalIndexInserter(index);
...
RC Table::update_record(Trx *trx, Record *record) {
  ...
  if (trx != nullptr) {
    ...
    if(rc == RC::SUCCESS) {
      ...
      if (rc2 != RC::SUCCESS) {
        LOG_ERROR("Failed to update index entry.");
        return rc2;
      }
    }
