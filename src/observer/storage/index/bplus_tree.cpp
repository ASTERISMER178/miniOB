...
RC BplusTreeHandler::create(const char *file_name, AttrType attr_type, int attr_length,
			    int internal_max_size /* = -1*/, int leaf_max_size /* = -1 */, bool is_unique)
{
  is_unique_ = false;
  ...
  key_comparator_.init(file_header->attr_type, file_header->attr_length, is_unique);
...
RC BplusTreeHandler::open(const char *file_name)
{
  ...
  key_comparator_.init(file_header_.attr_type, file_header_.attr_length, is_unique_);
