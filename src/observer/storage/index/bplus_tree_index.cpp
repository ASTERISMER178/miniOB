...
{
  if (inited_) {
    LOG_WARN("Failed to create index due to the index has been created before. file_name:%s, index:%s, field:%s, is_unique:%d",
        file_name,
        index_meta.name(),
        index_meta.field(),
  ...
  RC rc = index_handler_.create(file_name, field_meta.type(), field_meta.len(), -1, -1, index_meta_.is_unique());
  if (RC::SUCCESS != rc) {
